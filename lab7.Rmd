...
### tidy the data 
j2j_from_co_cln <- j2j_from_co %>% pivot_longer(cols = 2:ncol(j2j_from_co), names_to = "to_state", values_to = "avg_n_job")
```


```{r, class.source = "nocopy",  message=FALSE, warning=FALSE, cache=FALSE}
#### remove the record with ".flag". "!";    %like% operator.  find names containing the pattern ".flag"
j2j_from_co_cln <- j2j_from_co_cln %>% filter(!to_state %like% ".flag")
...
```{r, class.source = "nocopy",  message=FALSE, warning=FALSE, cache=FALSE}
#### re-order columns
j2j_from_co_cln <- j2j_from_co_cln[c("from_state", "to_state", "direction", "naics_sector", "avg_n_job")]
```
...
j2j_to_co_cln <-  j2j_to_co %>% 
  pivot_longer(cols = 2:ncol(.), names_to = "naics_sector", values_to = "avg_n_job" ) %>% 
...
plot(network, vertex.size = 12, 
     edge.arrow.size = 0.5,
     edge.width=E(network)$avg_n_job * 0.002, edge.curved = TRUE, main = "Job inflow to Colorado")
```
...
sf_us_state_j2j <- sf_us_state %>% filter(NAME %in% lt_state) %>% st_transform(., 2231)
```
...
for (i in 1:length(lt_sector)) {
  ...
  map_j2j(i, j2j_co_xy, sf_us_state_j2j, 
          df_node, "outflow_from_co", 
