...
Let's combine the two data frame ``` pop_cos ``` and ``` pop_buf ``` together and save it as a new variable ``` pop_cos_buf ```. Remember that we have checked their column names in Step 3.3.2. One condition to combine rows together is that they need to have the same column names.  
...
It seems the plot ``` plot1_growth ```  is good enough. Let's stop here for now and export it to the _"plot"_ folder we have created in [Step 3.2.1.]{.ul}. We will export it as a [.pdf]{.ul} file because it can store information in vector format and layers and is easier to edit later. 

> We will use the function ``` pdf() ```. The path ``` "plot/plot1_population_growth_two_cities.pdf" ``` tells the machine to save the pdf to the _"plot"_ folder and name it as _"plot1_population_growth_two_cities.pdf"_. Feel free to use a different name. 
