...
Going back to our data, let's check how many columns and rows are in the data ``` df_ep_poverty ```.
...
While the ``` df_ep_poverty ``` has 603 columns. But we only need some of them that have been explained in Step . Let's create a list storing the column names that we are interested in and are related to poverty. 
