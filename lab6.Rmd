...
And the distributions of the variables you are interested in, e.g., the "Agency" column. 
```{r, eval=FALSE, class.source = "nocopy"}
...
```{r, eval=FALSE, class.source = "nocopy"}
# convert to date 
# format code for dates: https://www.stat.berkeley.edu/~s133/dates.html
df_den_311$Case_Created_dttm   # Fri, 29 Dec 2023 19:20:08 GMT
df_den_311$case_create_date <- as.Date(df_den_311$Case_Created_dttm, format = "%a, %d %b %Y %H:%M:%S")

# **[Q]** What is the data type of column "case_create_date" 
str(df_den_311)
```
...
df_den_311$case_create_wk <- strftime(df_den_311$case_create_date, format = "%V") %>% as.numeric()
...
```{r, eval=FALSE, class.source = "nocopy"}
# Case summary 
df_311_summary <- table(df_den_311$Case_Summary) %>% as.data.frame() %>% arrange(-Freq)
View(df_311_summary)

# **[Q]** What are the top 5 categories of people's 311 service requests?
```


```{r, eval=FALSE, class.source = "nocopy"}
# only analyze 311 requests of the most mentioned categories 
...
```{r, eval=FALSE, class.source = "nocopy"}
# count 311 service requests in each category by week (one-hot encoding)
df_den_311_top <- dcast(df_den_311_top, case_create_wk ~ Case_Summary, 
                        fill = 0, value.var = "n",
...
```{r, eval=FALSE, class.source = "nocopy"}
# **[Q]** What information does the table "df_den_311_top" tell? 
View(df_den_311_top)
...
# **[Q]** Please compare the "df_den_311_top" before and after, how does the "tidy" process change the data structure? 
...
```{r, eval=FALSE, class.source = "nocopy"}
# viz: improve by changing color palette & add labels
...
```{r, eval=FALSE, class.source = "nocopy"}
# **[Q]** Which columns contain spatial information of 311 requests and could be used to convert "df_den_311" to spatial data?  
View(df_den_311)
...
## after projection, what are their projected CRS
...
![Figure 6.1: When four seasons start in 2024.](figs/lab6_season_week.png){width=80%}



![Figure 6.2: Line types in "ggplot2."](figs/lab6_ggplot2_linetype.png){width=50%}


![Figure 6.2: Bivariate color palettes in "pals".](figs/lab6_pals_bivariate_color_palette.png){width=70%}
